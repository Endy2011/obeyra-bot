...
const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        for (const user of usersToFetch) {
            inFlightDeviceFetch.set(user, fetchPromise.then(deviceMap => deviceMap[user] || []).catch(() => []));
        }
        try {
            await fetchPromise;
        }
            ...
            if (status === 406) {
                logger.warn({ err, toFetch }, 'getUSyncDevices got 406 (not-acceptable), proceeding with cached devices');
            } else {
                throw err;
            }
        }
            ...
            try {
                ...
                for (let i = 0; i < jidsRequiringFetch.length; i += BATCH_SIZE) {
                    ...
                    try {
                        ...
                        for (const jid of batch) {
                            inFlightSessionFetch.set(jid, batchPromise.then(() => undefined).catch(() => undefined));
                        }
                        ...
                        const status = err?.output?.statusCode || err?.statusCode;
                        if (status === 406) {
                            // 406 "not-acceptable" is a transient server rejection, NOT a JID-specific failure.
                            // Do NOT cache these JIDs as failed — the next attempt might succeed.
                            logger.warn({ err, batch }, 'session fetch got 406 (not-acceptable), will retry on next message');
                            for (const jid of batch) {
                                inFlightSessionFetch.delete(jid);
                            }
                            // Do NOT re-throw — let the flow continue with whatever sessions are available
                        } else {
                ...
                if (awaitingInflight.length) {
                    try {
                        ...
                        logger.warn({ err }, 'error waiting for in-flight session fetches');
                    }
                }
            } finally {
        ...
        const nodes = await Promise.all(patched.map(async (patchedMessageWithJid) => {
            ...
            try {
                ...
                const isSessionError =
                    ...
                    (err?.message && err.message.toLowerCase().includes('no sessions'));

                if (isSessionError) {
                    failedSessionFetchCache.set(jid, Date.now());
                    logger?.debug?.({ jid, err: err?.message }, 'no session for sub-device, suppressing future attempts');
                } else {
                    logger?.warn?.({ jid, err }, 'failed to encrypt message to participant, skipping');
                }
                return null;
            }
        }));
        // Filter out null nodes (those that failed to encrypt) and empty objects
        const validNodes = nodes.filter(node => node && Object.keys(node).length > 0);
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, isRetry }) => {
        ...
        if (!isRetry && wasMessageRecentlySent(msgId, jid)) {
        ...
        if (participant) {
            ...
            const { user, device, server } = (0, WABinary_1.jidDecode)(participant.jid);
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                const { ciphertext, senderKeyDistributionMessage } = await signalRepository.encryptGroupMessage({
                    ...
                    data: bytes,
                    meId: (groupData?.addressingMode === 'lid') ? (authState.creds.me.lid || meId) : meId,
                });
                const senderKeyJids = [];
                // ensure a connection is established with every device
                for (const { user, device, server } of devices) {
                    const targetServer = server || ((groupData === null || groupData === void 0 ? void 0 : groupData.addressingMode) === 'lid' ? 'lid' : 's.whatsapp.net');
                ...
                if (senderKeyJids.length) {
                    ...
                    try {
                        ...
                        logger.warn({ err, senderKeyJids }, 'assertSessions failed, proceeding with available sessions');
                    }
                    const result = await createParticipantNodes(senderKeyJids, senderKeyMsg, extraAttrs);
                    shouldIncludeDeviceIdentity = shouldIncludeDeviceIdentity || result.shouldIncludeDeviceIdentity;
                    participants.push(...result.nodes);
                    for (const node of result.nodes) {
                        if (node && node.attrs && node.attrs.jid) {
                            senderKeyMap[node.attrs.jid] = true;
                        }
                    }
                }
                ...
                const { user: meUser, server: meServer } = (0, WABinary_1.jidDecode)(meId);
                if (!participant) {
                    devices.push({ user, server }); // 'server' already defined from top of function
                    if (user !== meUser) {
                ...
                const meJids = [];
                const otherJids = [];
                for (const { user, device, server: fetchedServer } of devices) {
                    const isMe = user === meUser;
                    const targetServer = fetchedServer || (isLid ? 'lid' : 's.whatsapp.net');
                    const jid = (0, WABinary_1.jidEncode)(isMe && isLid ? ((_g = (_f = authState.creds) === null || _f === void 0 ? void 0 : _f.me) === null || _g === void 0 ? void 0 : _g.lid.split(':')[0]) || user : user, targetServer, device);
                    if (isMe) {
                        meJids.push(jid);
                    }
                    else {
                ...
                try {
                    ...
                    logger.warn({ err, allJids }, 'assertSessions failed for direct message, proceeding with available sessions');
                }
            ...
            try {
                await (0, retry_1.retryWithBackoff)(() => sendNode(stanza), {
                    ...
                    shouldRetry: (err) => {
                        const status = err?.output?.statusCode || err?.statusCode;
                        // Don't retry "not-acceptable" (406) errors
                        if (status === 406) {
                            return false;
                        }
                ...
                const status = err?.output?.statusCode || err?.statusCode;
                if (status === 406) {
                    logger.warn({ msgId, jid, participants: participants.length }, 'sendNode got 406 (not-acceptable), message delivery may have failed');
                } else {
                    throw err;
                }
            }
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'album' in content && content.album) {
                ...
                if (isDeleteMsg) {
                    ...
                    // if u see this, ts is stole by sum nga, original shi is made by realvare
                }
