...
const makeMessagesRecvSocket = (config) => {
    ...
    const normalizeContextInfoJids = async (contextInfo, groupJid) => {
        ...
        const normalizeJid = async (jid) => {
            ...
            if ((0, WABinary_1.isLid)(jid)) {
                // If someone mentions *us* using our LID, normalize it to our real JID.
                ...
                if (typeof myLid === 'string' && myLid) {
                    ...
                    if (myLidUser && jid === `${myLidUser}@lid`) {
                        return (0, WABinary_1.jidNormalizedUser)(authState.creds.me.id);
                    }
                }
                if (groupJid) {
        ...
        switch (child === null || child === void 0 ? void 0 : child.tag) {
            ...
            case 'leave':
                ...
                const participants = await Promise.all(participantNodes.map(async (p) => {
                    ...
                    // For ALL participant events, prefer phone_number or participant_pn over jid
                    if (phoneNumber) {
                        return typeof phoneNumber === 'string' ? (phoneNumber.includes('@') ? phoneNumber : `${phoneNumber}@s.whatsapp.net`) : phoneNumber;
                    }
                    if (pn) {
    ...
    const handleReceipt = async (node) => {
        ...
        // For private LID receipts, resolve via lidCache to match stored message keys
    ...
    const handleCall = async (node) => {
        ...
        // For private calls, also try sharedLidPhoneCache if LID wasn't resolved
