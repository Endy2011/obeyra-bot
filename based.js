...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${authFile}/creds.json`)) {
    do {
        // NUOVA PALETTE: ERROR-BOT STYLE (RED, MAGENTA & DARK GREY)
        const errRed = chalk.hex('#FF3333');
        const errMagenta = chalk.hex('#D8006E');
        const errDark = chalk.hex('#555555');
        const errCyan = chalk.hex('#00FFCC');
        const terminalText = chalk.hex('#CCCCCC');

        const borderTop = errRed('◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢ ERROR-BOT CONSOLE ◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥');
        const borderBot = errRed('◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥◣◥ TERMINAL END ◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢');
        const separator = errDark('═'.repeat(70));
        const header = chalk.bold.white('⚠️ INITIALIZATION_MENU_REQUIRED // SELEZIONE METODO ACCESSO');
        
        const optQr = errMagenta(' [⚙️_NODE_01] -> ') + chalk.bold.white('INJECT VIA SCANSIONE QR CODE');
        const optCode = errMagenta(' [⚙️_NODE_02] -> ') + chalk.bold.white('LINK VIA PAIRING CODE (8 CIFRE)');
        
        const infoLines = [
            errMagenta(' [!] ') + terminalText.italic('Immettere esclusivamente il valore numerico dell\'istanza.'),
            errMagenta(' [!] ') + terminalText.italic('Premere [ENTER] per confermare la pipeline.'),
            '',
            errDark(' >> System core architecture handled by error-bot v2.0'),
        ];
        const prompt = errCyan.bold('\n┌──(system@error-bot)─[~/auth_gateway]\n└─$ Inserisci opzione (1/2) ──► ');
...
${prompt}`);
        ...
        if (!/^[1-2]$/.test(opzione)) {
            console.log(`\n${chalk.bgRed.black.bold(' ❌ [CRITICAL_INPUT_ERROR]: VALORE FUORI MATRICE ')}
${errDark('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')}
${errRed.bold('⚠️ Stato instabile:')} ${terminalText('Consentiti solo i registri numerici')} ${chalk.bold.cyan('1')} ${terminalText('oppure')} ${chalk.bold.cyan('2')}
${errMagenta(' ⤷ [Eccezione]:')} ${terminalText('Stringhe o caratteri speciali non riconosciuti dal compilatore.')}
${errMagenta(' ⤷ [Risoluzione]:')} ${terminalText('Se riscontri crash di cifratura, contatta l\'amministratore: +393701330693')}
`);
        }
